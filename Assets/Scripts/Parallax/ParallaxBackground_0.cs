...
public class ParallaxBackground_0 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            // Reposition sprite when it goes out of view
            if (Layer_Objects[i].transform.position.x < _camera.position.x - boundSizeX * sizeX)
            {
                Layer_Objects[i].transform.position += Vector3.right * boundSizeX * sizeX * 2;
            }
        }
    }
